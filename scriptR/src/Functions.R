...
computeProfileGenesENS <- function( bed, wig, w = 3000, span = 200, seqlens , fun="sum",get="one"){
  if( class( wig ) != "SimpleRleList" ){
    stop( "ERROR : unknown class of wig, please provide a SimpleRleList" );
  }
  mat = NULL;
...
computeProfileTSSENS <- function( bed, wig, w1 = 3000,w2 = 3000, span = 200, seqlens , fun="sum",get="one"){
  if( class( wig ) != "SimpleRleList" ){
    stop( "ERROR : unknown class of wig, please provide a SimpleRleList" );
  }
  mat = NULL;
  ...
  if(get == "one"){
    rv = switch(fun,
                "sum"=colSums( mat ,na.rm = T),
                "mean"=colMeans( mat,na.rm = T ));
  }else{
    rv = mat
  }
  return( rv );
